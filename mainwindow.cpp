    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->pushButton_card,SIGNAL(clicked()),this,SLOT(show_card()));
    connect(ui->pushButton_card,SIGNAL(clicked()),this,SLOT(insert_combo()));
    connect(ui->comboBox,SIGNAL(currentIndexChanged(QString)),this,SLOT(use_card(QString)));
    connect(ui->start,SIGNAL(clicked()),this,SLOT(start_zhua()));
    //connect(ui->start,SIGNAL(clicked()),this,SLOT(end_zhua()));
}
...
void MainWindow::show_card()
{
    ...
    for (d = alldevs; d != NULL; d = d->next)
        {
            string1=QString::number(n);
            ui->Showcard->insertPlainText(string1+" ");
            n++;
            ui->Showcard->insertPlainText(d->name);
            ui->Showcard->insertPlainText(d->description);
            ui->Showcard->insertPlainText("\n");

        }
    n=1;
}
void MainWindow::insert_combo()
{
    if (pcap_findalldevs(&alldevs, errbuf) == -1)
    {
        fprintf(stderr, "Error in pcap_findalldevs: %s\n", errbuf);
    ...
    ui->comboBox->addItem("-----请选择要zhua的网卡-----");
    for (d = alldevs; d != NULL; d = d->next)
        {
...
void MainWindow::use_card(QString)
{
    int n=ui->comboBox->currentIndex();
    for (d = alldevs, i = 0; i < n - 1; ++i, d = d->next);
    printf("%d ",n);
}

int MainWindow::start_zhua()
{
    int numb_bao=0;
    ...
    if ((freopen_s(&file, "data.txt", "w", stdout)) == 0);
            ...
            while ((res = pcap_next_ex(fp, &header, &pkt_data)) >= 0)
            {
                ...
                localtime_s(&ltime2, &local_tv_sec);
                strftime(timestr, sizeof(timestr), "%H:%M:%S", &ltime2);
